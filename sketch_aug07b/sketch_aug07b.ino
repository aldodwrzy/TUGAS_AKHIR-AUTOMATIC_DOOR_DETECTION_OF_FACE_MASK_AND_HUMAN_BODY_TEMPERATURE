const char* ssid     = "SamsungA8+";   //your network SSID
...
#define RESTART_SCAN 'n'
  ...
  while (Serial.available() > 0)
  {
    ...
    if (recv.indexOf("a") != -1) {
      ...
      {
        if (P1 == "Mask") {
          Serial.println("*100,");       //send percentage number to Arduino
          start = true;
          break;
        }

        if (millis()-StartTime > 5000) {
          Serial.println("*0,");       //send percentage number to Arduino
          break;
        }
      }
    }
    ...
    if (usRead_Serial_Data(szData, sizeof(szData)) > 0) //read data request from Arduino
    {
      if (szData[0] == START_RUN) // start face_detection algorithm
      {
        ...
        while ( start == false)
        {
          ...
          if (P1 != "Mask" && millis() - StartTime > 10000) {
            Serial.println("*0,");       //send percentage number to Arduino
            break;
          }
        }
...
<html>
    <head>
        ...
        <title>Face Recognition & Mask Detection</title>
        <style>
        ...
        <section class="main">
            ...
            <section id="buttons">
                ...
                <tr>
                  ...
                  <td><button id="toggle-stream" style="display:none"></td>
                </tr>
                </table>
            </section>        
            <div id="logo">
